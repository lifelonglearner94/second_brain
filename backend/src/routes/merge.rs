...
pub async fn approve(State(state): State<AppState>, Path(id): Path<i64>) -> Result<StatusCode> {
    graph::approve_merge_suggestion(&state.db, id).await?;
...
pub async fn reject(State(state): State<AppState>, Path(id): Path<i64>) -> Result<StatusCode> {
    graph::reject_merge_suggestion(&state.db, id).await?;
