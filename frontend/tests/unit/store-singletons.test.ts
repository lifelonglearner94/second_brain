...
import { SpatialGraphStore, spatialGraph } from '../../src/lib/state/spatial-graph.svelte';
...
describe('store singletons — one shared instance exported from each store module (the spatial-graph pattern)', () => {
	...
	it('spatialGraph is the shared SpatialGraphStore singleton exported by the store module (existing pattern)', () => {
		expect(spatialGraph).toBeInstanceOf(SpatialGraphStore);
	});
	...
	it('each singleton is the same instance across re-imports (module-cached, not a fresh one per import)', async () => {
		const reloaded = await Promise.all([
			...
			import('../../src/lib/state/spatial-graph.svelte')
		]);
		...
		expect(reloaded[5].spatialGraph).toBe(spatialGraph);
	});
});
